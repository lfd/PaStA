...
**PaStA**  is a tool for detecting similar patches. They can come from patch stacks (for example different branches within a git repository) or mailboxes (many OSS projects, like the Linux kernel, use a mail-based workflow). In general, a patch stack (also known as patch set) is defined as a set of patches (commits) that are developed and maintained independently of the base project.
...
Tracking of Commits on Mailing Lists](https://arxiv.org/pdf/1902.03147.pdf)

Getting PaStA
-------------

Clone **PaStA** and its resources submodule. The resources contain configuration, repositories, public inboxes as well as results of **PaStA**'s analysis for some sample projects.
...
#### Mailbox Mode
The development of a number of open source projects is carried out on mailing lists.
A prime example of this is the Linux kernel. Developers send patches to mailing lists, where they are reviewed by other subscribers or maintainers.
Note that a project may use several mailing lists. The Linux kernel, for example, uses more than 200 different lists for its subsystems. Discussion related to new features, bug reports etc. also takes place on the
mailing list.

Given a mailbox of such a mailing list, PaStA can parse all mails that contain patches. It can also map these patches to their corresponding upstream commits.
...
#### Patch stack mode
This mode uses patch stack definition files to compare succesive versions of patch stacks.
The patch stack definition file can be generated by a script. Eg. [PreemptRT project's script](https://github.com/lfd/PaStA-resources/blob/67ee076cbcb0f157cf045532471e1f886245e1ba/PreemptRT/update-preemptrt.sh)
