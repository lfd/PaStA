...
def upstream_duration(date_selector, patch_group):
    first_stack_relase = min(map(lambda x: date_selector(x), patch_group))
    upstream_date = get_commit(patch_group.property).commit_date

    delta = first_stack_relase - upstream_date
    return delta


def pasta_upstream_history(prog, argv):
    ...
    upstream_helper = functools.partial(upstream_duration, date_selector)
    ...
    for group, duration in upstream_groups:
        upstream = get_commit(group.property)
...
if __name__ == '__main__':
    pasta_upstream_history(sys.argv[0], sys.argv[1:])
